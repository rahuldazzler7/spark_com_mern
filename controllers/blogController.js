...
let createPost = (req, res) => {
  ...
  if (!title || !description || !type) {
    ...
    console.log("I am auth  1>>" + req.user);
            ...
            if (postr) {
              ...
              newPost
                ...
                .then((newpost) => {
                  res.json({
                    status: true,
                    msg: "Your post is successfully uploaded",
                  });
...
let getEditblog = (req, res) => {
  ...
  blogpost.findById(req.params.id, (err, ptos) => {
    if (err) {
      ...
      res.json({ status: true, rposts: ptos });
      //console.log(ptos)
    }
    // } else {
        ...
        if (err) {
          ...
          blogpost.findByIdAndUpdate(req.params.id, updateddata, (err) => {
            if (err) {
              ...
              res.json({ status: true, msg: "Post is updated" });
            }
          });
        }
      }
...
module.exports = { createPost, upload, editPost, getEditblog, deletePost };
